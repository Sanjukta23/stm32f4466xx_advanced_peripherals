...
static volatile uint32_t g_mpu6050_unhandled_int_count = 0;
static volatile uint8_t  g_mpu6050_last_unhandled_int  = 0;
...
void mpu6050_interrupt_handle(I2C_HandleTypeDef *hi2c) {
	...
	if (get_interrupt_status(hi2c, &int_status) != MPU6050_OK) return;
	if (get_interrupt_setting(hi2c, &int_settings) != MPU6050_OK) return;

	uint8_t active = int_status & int_settings;   // enabled AND pending

	if (active & FF_INT)      mpu6050_free_fall_detection_callback();
	if (active & MOT_INT)     mpu6050_motion_detection_callback();
	if (active & ZMOT_INT)    mpu6050_zero_motion_detection_callback();
	if (active & RAW_RDY_INT) mpu6050_raw_data_ready_callback();
	if (active & DMP_INT)     mpu6050_dmp_callabck();

	if (active & FIFO_OFLOW_INT) {
		// FIFO overflowed: data stream is corrupt, reset it
		mpu6050_fifo_reset(hi2c);
		mpu6050_fifo_overflow_callback();
	}

	// Anything pending that we didn't expect (not enabled, or unhandled source):
	// count it so it's visible in the debugger instead of vanishing silently
	uint8_t unhandled = int_status & (uint8_t)~(FF_INT | MOT_INT | ZMOT_INT |
	                                            RAW_RDY_INT | DMP_INT | FIFO_OFLOW_INT);
	if (unhandled) {
		g_mpu6050_unhandled_int_count++;
		g_mpu6050_last_unhandled_int = unhandled;
	}
}
...
__weak void mpu6050_fifo_overflow_callback(void) {
